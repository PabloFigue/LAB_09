...
void __interrupt() isr(void)
{
    ...
    { 
            ...
            else if (PORTBbits.RB1 == 0){ //----> Despierta al PIC
                sleeping = 0;
                __delay_ms(1000);
            }
            else if (PORTBbits.RB2 == 0){ //----> Despierta al PIC
                sleeping = 0;
                __delay_ms(1000);
            }
        INTCONbits.RBIF = 0;
...
void main(void) 
{
    ...
    { 
        if (sleeping == 0){ //Revision si no esta en el modo SLEEP
            if (ADCON0bits.GO == 0){    //Revisión si el ADC no está funcionando
                ADCON0bits.GO = 1;      //Iniciar a convertir
                __delay_ms(2);
            }
            PORTE = cont_sleep; //Contador mientras no esta en SLEEP
            cont_sleep ++;
            if (cont_sleep>7)
            cont_sleep = 0;
        } 
